...
def pdf_attack_report(attack_id, user_id):
    ...
    # Look for the attack in MongoDB that matches the attack id and user id
    attack = collection_attacks.find_one({
        "_id": ObjectId(attack_id),
        "user_id": ObjectId(user_id)
    })

    # If the attack doesn't exist or doesn't belong to this user, return nothing
    if not attack:
        return None

    # Create a buffer in memory where the PDF will be stored
    pdf_buffer = BytesIO()

    # Create the PDF document using ReportLab
    pdf_doc = SimpleDocTemplate(pdf_buffer)

    # List that will hold all elements that go into the PDF
    flowables = []
    ...
    # Add attack details section
    flowables.append(Paragraph("Attack Details", style_section))

    # Fields we don't want to show in the report
    ignore_keys = {"_id", "user_id"}

    # Loop through all attack fields and add them to the PDF
    for key, val in attack.items():
        ...
        # Format the field name to look nicer in the report
        label = key.replace("_", " ").title()

        # Add the field and its value
    ...
    # Move buffer pointer to the start so Flask can send it
    pdf_buffer.seek(0)

    # Create a dynamic filename for the report
